...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = np.multiply(dnext_h, 1 - np.power(next_h, 2))  # N x H
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dWh.append(dWh_t)
        db.append(db_t)
    dx = np.stack(reversed(dx), axis=1)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = prev_h, prev_c, x, next_h, next_c, i, f, o, g, Wx, Wh, b
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    prev_h, prev_c, x, next_h, next_c, i, f, o, g, Wx, Wh, b = cache
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    probs /= np.sum(probs, axis=1, keepdims=True)  # (N * T)  x V
